...
class DataGenerator():
    ...
    def _read_img_dir_from_csv(self):
        ...
        self.coordinates_outwear = []
        self.coordinates_skirt = []
        self.coordinates_trousers = []

        with open(self.train_data_file) as csv_file:
    ...
    def _create_train_table(self, category):
        ...
        for i in range(len(images)):
            ...
            x = []
            y = []
            for x_y_v in x_y_visi:
                x_y_v_split = x_y_v.split('_')  # 按_分割
                x.append(int(x_y_v_split[0]))
                y.append(int(x_y_v_split[1]))
                w.append(int(int(x_y_v_split[2]) >= 0))  # weights = 0 不存在 1 存在或不可见

            joints = []
            for index in range(0, len(x)):
                joints.append([x[index], y[index]])

            if self.joints_num != len(joints):
    ...
    def _create_sets(self, validation_rate=0.1):
        ...
        print('SET CREATED')
        if self.category == 'blouse':
            np.save('Dataset-blouse-Validation-Set', self.valid_set)
            np.save('Dataset-blouse-Training-Set', self.train_set)
        elif self.category == 'dress':
            np.save('Dataset-dress-Validation-Set', self.valid_set)
            np.save('Dataset-dress-Training-Set', self.train_set)
        elif self.category == 'outwear':
            np.save('Dataset-outwear-Validation-Set', self.valid_set)
            np.save('Dataset-outwear-Training-Set', self.train_set)
        elif self.category == 'skirt':
            np.save('Dataset-skirt-Validation-Set', self.valid_set)
            np.save('Dataset-skirt-Training-Set', self.train_set)
        elif self.category == 'trousers':
            np.save('Dataset-trousers-Validation-Set', self.valid_set)
            np.save('Dataset-trousers-Training-Set', self.train_set)
    ...
    def _crop_data(self, height, width, joints, boxp=0.05):
		...
		"""
        padding = [[0, 0], [0, 0], [0, 0]]
        # box = [x_min,y_min,x_max,y_max], x_min = min(joints_x)
        '''
        j = np.copy(joints)
        if box[0:2] == [-1, -1]:
            j[joints == -1] = 1e5
        ...
        '''
        crop_box = [0, 0, width - 1, height - 1]    # no box
        new_h = int(crop_box[3] - crop_box[1])
        new_w = int(crop_box[2] - crop_box[0])
        crop_box = [crop_box[0] + new_w // 2, crop_box[1] + new_h // 2, new_w, new_h]  # [中心x,中心y,宽,高]
        if new_h > new_w:
    ...
    def _augment(self, img, hm, max_rotation=30):
        ...
        if random.choice([0, 1]):
            ...
            img = transform.rotate(img, r_angle, resize=False)
            hm = transform.rotate(hm, r_angle, resize=False)
        return img, hm
    ...
    def _generator(self, batch_size=16, stacks=4, set='train', stored=False, normalize=True, debug=False):
        ...
        if self.category == 'blouse':
            ...
            print('_generator error: category error')
            exit(-1)
    ...
    def _aux_generator(self, batch_size=16, stacks=4, normalize=True, sample_set='train'):
		...
		Args:
			...
			batch_size	    : Number of images per batch
			stacks			: Number of stacks/module in the network
			sample_set		: Training/Testing/Validation set
			normalize		: True to return Image Value between 0 and 1
		"""
        while True:
            train_img = np.zeros((batch_size, 256, 256, 3), dtype=np.float32)
            ...
            while i < batch_size:
                try:
                    ...
                    new_j = self._relative_joints(cbox, padd, joints, to_size=64)
                    # print('_relative_joints done')
                    hm = self._generate_hm(64, 64, new_j, 64, weight)
                    img = self._crop_img(img, padd, cbox)
                    # print('_crop_img done')
                    img = img.astype(np.uint8)
                    img = scm.imresize(img, (256, 256))
                    img, hm = self._augment(img, hm)
                    # print('_augment done')
    ...
    def test(self, toWait=0.2):
        ...
        for i in range(len(self.train_set)):
            ...
            padd, box = self._crop_data(img.shape[0], img.shape[1],
