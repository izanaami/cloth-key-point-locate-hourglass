...
class HourglassModel():
	...
	def __init__(self,
		...
		Args:
			...
			modif				: (bool) Boolean to test some network modification
									# DO NOT USE IT ! USED TO TEST THE NETWORK
	...
	def dist_Euclid(self, x1, y1, x2, y2):
		...
		y_1 = tf.to_float(y1)
		y_2 = tf.to_float(y2)
		x_diff_square = tf.square(tf.subtract(x_1, x_2))
		y_diff_square = tf.square(tf.subtract(y_1, y_2))
		dist = tf.sqrt(tf.add(x_diff_square, y_diff_square))
		return dist

	def max_coordinate(self, array):
		"""
		返回一个矩阵最大/小值的坐标
		Args:
			array:
		...
		Returns:
			...
			y:
				...
				Type: tf.int32
		"""
		list = tf.reshape(array, shape=[-1])
		xy = tf.argmax(list)
		xy = tf.to_int32(xy)

		x = tf.divide(xy, 64)  # x = xy//64
		x = tf.floor(x)
		x = tf.to_int32(x)

		y = tf.subtract(xy, tf.to_int32(tf.multiply(x, 64)))  # y = xy - x*64
		y = tf.to_int32(y)
		return x, y
	...
	def normalized_error(self):
		...
		Args:
			self.output:    网络输出
				Shape: (batch_size, nbStacks, 64, 64, outputDim)
				Type : tf.float32
			self.gtMaps:    标记数据
				Shape: (batch_size, nbStacks, 64, 64, outputDim)
				Type : tf.float32
			self.weights:   标记点是否存在
				Shape: (batch_size, outputDim)
				Type : tf.float32
		Returns:

		"""
		if self.outDim == 13 or self.outDim == 15:  # blouse dress
			normal_left = 5
			normal_right = 6
		elif self.outDim == 14:  # outwear
			normal_left = 4
			normal_right = 5
		elif self.outDim == 4:  # skirt
			normal_left = 2
			normal_right = 3
		elif self.outDim == 7:  # trousers
			normal_left = 4
			normal_right = 6
		...
		for i in range(int(self.batchSize)):
			# 求归一化点距离
			normalmap_left = self.gtMaps[i, self.nStack - 1, :, :, normal_left]
			normalmap_right = self.gtMaps[i, self.nStack - 1, :, :, normal_right]
			left_x, left_y = self.max_coordinate(normalmap_left)
			right_x, right_y = self.max_coordinate(normalmap_right)
			normalize_dist = self.dist_Euclid(left_x, left_y, right_x, right_y)

			# 求出预测关键点和标注关键点之间的距离 d_k
			sum_d_s = 0.0  # weight为1的joint距离差和归一化距离之比的和
			num_k = 0  # weight为1的joint数
			for k in range(int(self.outDim)):
				if tf.equal(self.weights[i, k], 1.0) is not None:  # 当weight为1时才计算距离
					output = self.output[i, self.nStack - 1, :, :, k]
					gtMap = self.gtMaps[i, self.nStack - 1, :, :, k]
					output_x, output_y = self.max_coordinate(output)
					gtmap_x, gtmap_y = self.max_coordinate(gtMap)
					dist = self.dist_Euclid(output_x, output_y, gtmap_x, gtmap_y)
					sum_d_s = tf.add(sum_d_s, tf.divide(dist, normalize_dist))
					num_k = tf.add(num_k, 1)
			num_k = tf.to_float(num_k)
			sum_NE = tf.add(sum_NE, tf.divide(sum_d_s, num_k))
		len = tf.to_float(self.batchSize)
		NE = tf.divide(sum_NE, len)
		return NE
		...
		with tf.device(self.gpu):
			...
			with tf.name_scope('loss'):
				if self.w_loss:
					...
					self.loss = tf.nn.l2_loss(tf.subtract(self.output, self.gtMaps), name='cross_entropy_loss')
			lossTime = time.time()
	...
	def restore(self, load=None):
		...
		with tf.name_scope('Session'):
			with tf.device(self.gpu):
				self._init_session()
				self._define_saver_summary(summary=self.w_summary)
				if load is not None:
	...
	def _train(self, nEpochs=10, epochSize=1000, saveStep=500, validIter=10, category=None):
		"""
		"""
		with tf.name_scope('Train'):
			self.generator = self.dataset._aux_generator(self.batchSize,
														 self.nStack,
														 normalize=True,
														 sample_set='train')
			self.valid_gen = self.dataset._aux_generator(self.batchSize,
														 self.nStack,
														 normalize=True,
														 sample_set='valid')
			startTime = time.time()
			...
			for epoch in range(nEpochs):
				...
				avg_cost = 0.
				cost = 0.
				print(category + 'Epoch :' + str(epoch) + '/' + str(nEpochs) + '\n')
				# Training Set
				for i in range(epochSize):
					...
					sys.stdout.write(
						...
						+ str(percent)[:4] + '%' + ' -cost: ' + str(cost)[:6]
						+ ' -avg_loss: ' + str(avg_cost)[:5] + ' -timeToEnd: ' + str(tToEpoch) + ' sec.')
					sys.stdout.flush()
				...
				print(category + 'Epoch ' + str(epoch) + '/' + str(nEpochs) + ' done in ' + str(
	...
	def training_init(self, nEpochs=10, epochSize=1000, saveStep=500, dataset=None, load=None, category=None):
		...
		with tf.name_scope('Session'):
			with tf.device(self.gpu):
				...
				self._train(nEpochs, epochSize, saveStep, validIter=10, category=category)
	...
	def weighted_bce_loss(self):
		...
		e1 = tf.expand_dims(self.weights, axis=1, name='expdim01')  # batch_size, 1, outputdim
		e2 = tf.expand_dims(e1, axis=1, name='expdim02')  # batch_size, 1, 1, outputdim
		e3 = tf.expand_dims(e2, axis=1, name='expdim03')  # batch_size, 1, 1, 1, outputdim
		return tf.multiply(e3, self.bceloss, name='lossW')
		'''
		diff = tf.subtract(self.output, self.gtMaps)  # (batch_size, nbStacks, 64, 64, outputDim)
		e1 = tf.expand_dims(self.weights, axis=1, name='expdim01')  # batch_size, 1, outputdim
		e2 = tf.expand_dims(e1, axis=1, name='expdim02')  # batch_size, 1, 1, outputdim
		e3 = tf.expand_dims(e2, axis=1, name='expdim03')  # batch_size, 1, 1, 1, outputdim
		weight_diff = tf.multiply(e3, diff)
		bceloss = tf.nn.l2_loss(weight_diff, name='bce_loss')
		bceloss = tf.to_float(bceloss)
		num = self.batchSize * self.nStack *self.outDim
		bceloss = tf.divide(bceloss, float(num))
		return bceloss
