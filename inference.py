...
class Inference():
	...
	def centerStream(self, img):
		img = cv2.flip(img, 1)
		img[:, self.predict.cam_res[1]//2 - self.predict.cam_res[0]//2:self.predict.cam_res[1]//2
	...
	def plotLimbs(self, img_res, j):
		...
		for i in range(len(self.predict.links)):
			...
			if good_link:
				...
				cv2.line(img_res, tuple(pos[0])[::-1], tuple(pos[1])[::-1],
						 self.predict.links[i]['color'][::-1], thickness = 5)
